...
├── final/
│   └── codyssey_ad_bonus1.mp4          # 보너스1
│   └── codyssey_ad_bonus3.mp4          # 보너스3
├── source/
│   ├── video/
│   │   ├── scene1.mp4                 # 씬1 (Kling)
...
| 이미지 생성 | **Gemini** (Nano Banana Pro) | 립싱크용 인물(정장 남성) 포트레이트 (보너스1) | 무료 티어에서 얼굴·조명이 자연스럽고 아티팩트 적음 | 세밀한 스타일 고정은 제한 | Bing, Leonardo, Firefly |
...
| 립싱크 | **Hedra** | 씬3 인물 발화 영상 생성 (보너스1) | 사진+오디오로 표정까지 살아있는 토킹헤드, 립싱크 정확도 우수 | 무료 티어 720p 제한 | LipSync.video, HeyGen |
...
- [x] **보너스 1 – 립싱크(Lip-sync)** ✅ 완료
- [ ] **보너스 2 – 동일 스토리보드, 다른 도구로 재제작** (미진행)
- [x] **보너스 3 – 플랫폼별 화면 비율 버전** ✅ 완료

### 보너스 1 — 립싱크 (Lip-sync)

- **목표**: 인물이 직접 말하는 발화 장면을 추가해 메시지 전달력을 높인다.
- **접근**: 기존 씬3 내레이션 "누구나 만들 수 있습니다"(`source/narration/03_누구나_만들_수_있습니다.mp3`)를 **재활용** — 새 대사를 만들지 않아 톤을 유지하고 크레딧을 절약.
- **제작 과정**:
  1. **얼굴 이미지 생성 (Gemini)** — 중저음 남성 톤에 맞춰 정장 차림의 정면 인물 포트레이트를 생성. 립싱크 인식을 위해 정면·입 다문 표정·고른 조명 조건을 프롬프트에 반영.
  2. **립싱크 영상 생성 (Hedra)** — 위 이미지 + 기존 내레이션 오디오를 입력해 토킹헤드 영상 생성. (Kling 크레딧 소진으로 Hedra 채택, 무료 720p)
  3. **CapCut 통합** — 항해 컷(씬3)을 인물 발화 컷으로 교체하고, 해당 구간의 기존 내레이션 트랙은 음소거하여 음성 중복을 방지. BGM·자막은 그대로 유지.
- **결과물**: 립싱크 확장 버전 (파일명·위치는 리포지토리 구조 참조)

### 보너스 3 — 플랫폼별 화면 비율 버전

- **목표**: 동일 스토리보드로 화면 비율 버전을 2개 이상 확보한다.
- **제작 과정**:
  - 보너스1 립싱크 확장본을 소스로 **1:1 (1080×1080, 피드용)** 버전 제작 → 기존 **16:9**와 합쳐 총 2개 비율 확보.
  - 16:9 → 1:1 변환 시 위아래 여백이 생기므로 클립을 Scale로 확대해 정사각을 꽉 채우고, 씬별 Position을 키프레임으로 조정해 피사체·자막·로고가 잘리지 않도록 리프레임.
- **결과물**: 1:1 비율 버전 (파일명·위치는 리포지토리 구조 참조)
